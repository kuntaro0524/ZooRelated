#!/bin/bash
ROOTDIR=$PWD
SCRIPT_DIR=/staff/bl32xu/Staff/kuntaro/171101-PH/_kamoproc_dials/merge_scripts/merge_10deg_re/ccc_1.1A_framecc_b+B/scripts/
...
# Using model: water pdb
FINAL_PDB=$PROCPATH/water_001.pdb

# 環境変数を引き継ぎつつ、refine_template.comを実行
# backgroundで実行し、48つのジョブが終了するまで待つ
export ORIMTZ PROCPATH SYMM FREEMTZ MODEL_PDB REFMTZ RESOL FINAL_PDB
envsubst < $SCRIPT_DIR/16_phaser_input.com | bash &
