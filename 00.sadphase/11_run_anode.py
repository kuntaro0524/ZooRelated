    ...
    for index, col in df.iterrows():
        ...
        if existsSolved:
            if count_o >= count_i:
                ...
                target_phs = phs_i
            # phs2mtz 
            orig_phs=os.path.join(sad_proc_path, target_phs)
            new_phs = os.path.join(sad_proc_path,"anode.phs")
            os.system("cp -Rf %s %s" % (orig_phs, new_phs))

            # Preparing required files for anode
            # PDB file name should be identical to 'prefix' of SHELX
            pdb_name = os.path.join(sad_proc_path, "anode.pdb")
            os.system("cp -Rf %s %s" % (model, pdb_name))

            # FA file for ANODE
            fa_file = os.path.join(sad_proc_path, "%s_fa.hkl" % prefix)
            anode_fa_file=os.path.join(sad_proc_path, "anode_fa.hkl")
            os.system("cp -Rf %s %s" % (fa_file, anode_fa_file))

            comf = ComRefine.ComRefine(sad_proc_path,qhost_selection_flag=False)
            comname = comf.anode("anode")
            os.system("chmod a+x %s" % comname)
            os.system("qsub %s" % comname)
            # Generating phasing succeeded file.
            result_file=os.path.join(sad_proc_path, ".ANODE")
