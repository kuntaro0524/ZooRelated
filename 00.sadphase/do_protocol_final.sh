...
while [ 1 ]
do
n_finished=`ls merge*/ | grep ccp4 | wc -l`
echo "finished " $n_finished
if [ $n_finished -ge 80 ] ; then
break
else
    echo "waiting cycle=$n_cycle done=$n_finished"
    let n_cycle++
    if [ $n_cycle -ge 50 ]; then
        echo "Limitation"
        break
    fi
    sleep 10.0
...
while [ 1 ]
do
n_finished=`grep "SHELXE finished" merge*/00.SAD/shelxe_i.log | wc -l`
echo "finished " $n_finished
...
while [ 1 ]
do
n_finished=`ls merge*/00.SAD/mapcc.dat | wc -l`
echo "finished " $n_finished
